...
import { FaAngleRight, FaLinkedin, FaSquareFacebook, FaSquareTwitter } from 'react-icons/fa6';
...
class ProfileComponent extends BandungComponent {
			...
			return (
				<>
					<div className='flex flex-col font-700 text-qgray border-[1px] border-[solid] rounded-[10px]'>
						<div className='m-[15px]'>
							<span className='mb-[10px] text-qblack'>Personal Information</span>
	...
	executeBioUpdate(event) {
		...
		form.append(
			'longDescription',
			draftToHtml(convertToRaw(this.longDescriptionEditorState.getCurrentContent()))
		);
		Axios.post(`${getPrefix()}/app/user/update`, form).then(response => {
